...
public class SessionManagementTestCase {
    ...
    public void testSessionManagementOperations() throws Exception {
        try (CloseableHttpClient client = HttpClients.createDefault()) {
            ...
            final List<ModelNode> valList = resultList.stream().filter(e -> "val".equals(e.asString())).collect(Collectors.toList());
            Assert.assertEquals(1, valList.size());
            Assert.assertEquals(opRes.toString(), "val", valList.get(0).asString());
            final List<ModelNode> websocketList = resultList.stream().filter(e -> UNDERTOW_SESSION_ATTRIBUTE.equals(e.asString())).collect(Collectors.toList());
            ...
            final List<Property> valPropertyList = properties.stream().filter(e -> "val".equals(e.getName())).collect(Collectors.toList());
            ...
            final List<Property> websocketPropertyList = properties.stream().filter(e -> UNDERTOW_SESSION_ATTRIBUTE.equals(e.getName())).collect(Collectors.toList());
